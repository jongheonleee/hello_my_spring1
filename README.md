...
> - 단순하게 보면 '어떤 경계 안에 있는 내부 구성 요소들이 어떤 책임을 갖고 있고, 어떤 방식으로 서로 관계를 맺고 동작하는지를 규정하는 것'
> - 아키텍처는 정적인 구조를 나타내는 것으로만 생각하기 쉽지만 실제로는 그 구조에서 일어나는 동적인 행위와 깊은 관계가 있음

<br>

#### 👉 계층형 아키텍처

> - 관심, 책임, 성격이 변하는 이유와 방식이 서로 다른 것들을 분리함으로써 분리된 각 요소의 응집도는 높여주고 서로 결합도를 낮추는 것이 핵심
> - 변경이 발생할 경우, 해당 부분과 관련된 경계 내에서 변경작업을 효율적으로 처리해줘야함
> - 따라서, 인터페이스와 같은 유연한 경계를 만들어 두고 분리하거나 모아주는 작업이 필요함. 즉, 관련도 높은 것들끼리 그룹핑해주고 다른 부분을 분리하는 작업이 중요함

<br>

#### 👉 아키텍처와 SoC

> - 오브젝트 레벨에서 해온 DI를 기반으로 한 유연한 설계와 구현 전략이 아키텍처 레벨에서 좀 더 큰 단위에 대해서도 동일하게 적용할 수 있음
> - 애플리케이션을 구성하는 오브젝트들을 비슷한 성격과 책임을 가진 것들끼리 묶을 수 있음
> - 성격이 다른 것은 아키텍처 레벨에서 분리하는 것이 좋음. 이렇게 분리된 각 오브젝트는 독자적으로 개발과 테스트가 가능해서 개발과 변경 작업이 모두 빨라질 수 있음 
> - 또, 구현 방법이나 세부 로직은 서로 영향을 주지 않고 변경될 수 있게 함
> - 이렇게 '책임'과 '성격'이 다른 것을 크게 그룹으로 만들어 분리해두는 것을 아키텍처 차원에서는 '계층형 아키텍처'라고함
> - 보통 웹 기반의 엔터프라이즈 애플리케이션은 일반적으로 세 개의 계층을 갖는다고 해서 '3계층 애플리케이션'이라고 함
...
> - <3계층 아키텍처>
> - 3계층 아키텍처는 '데이터 액세스 계층', '서비스 계층', '프리젠테이션 계층'으로 나뉨
>   - (1) 데이터 액세스 계층 : 백엔드의 DB나 레거시 시스템과 연동하는 인터페이스 역할을 함
>   - (2) 서비스 계층 : 비즈니스 로직을 담고 있는 계층
>   - (3) 프리젠테이션 계층 : 웹 기반의 UI를 만들어 내고 그 흐름을 관리하는 계층
...
> - <두개 이상의 서비스를 통합하는 추상 계층의 도입>
> - 데이터 액세스 계층은 장기적인 데이터 저장을 목적으로 하는 DB이용이 주된 책임이됨
> - 또 외부 시스템을 호출해서 서비스를 이용하는 것은 기반(infrastructure) 계층으로 따로 분류하기도함
> - 데이터 액세스 계층은 사용 기술에 따라서 다시 세분화된 계층으로 구분될 수 있음
> - 데이터 액세스 계층 안에서 다시 세분화하는 경우는 추상화 수준에 따른 구분이기 때문에 '수직적인 계층'이라고도 불림
> - 3계층은 기술 계층보다는 역할에 따라 구분한 것이므로 보통 그림으로 나타낼때도 가로로 배열함
> - 반면에 같은 책임을 가졌지만 추상화 레벨에 따라 구분하는 경우는 세로로 배열해서 표현함
> - 새로운 추상 계층을 도입해서 하위 계층의 종류가 다른 서비스를 일관된 방식으로 접근할 수 있게 만들 수 있음
...
> - DAO 계층을 호출하고 이를 활용해서 만들어짐. 때론 데이터 액세스를 위한 기능 외에 서버나 시스템 레벨에서 제공하는 기반 서비스를 활용할 필요도 있음 
> - 서비스 계층은 특별한 경우가 아니라면 추상화 수직 계층구조를 가질 필요가 없음
> - 위에 그림에서 눈여겨봐야할 부분은 기반 서비스 계층이 서비스 계층의 오브젝트를 호출하는 경우임
> - 일반적으로는 서비스 계층이 필요에 따라 기반 서비스 계층의 API를 호출해서 이용함
> - 하지만, 서비스 계층의 코드를 기반 서비스 계층에서 실행시키는 경우도 있음
> - 원칙적으로 서비스 계층 코드가 기반 서비스 계층의 구현에 종속되면 안됨
> - 이상적인 서비스 계층은 데이터 액세스 계층이 바뀌고 프리젠테이션 계층이 모두 바뀌어도 그대로 유지될 수 있어야함

<br>

#### 👉 프리젠테이션 계층

> - 클라이언트와 연결되는 계층으로 매우 다양한 기술과 프레임워크의 조합을 가질 수 있음
> - 엔터프라이즈 애플리케이션의 프리젠테이션 계층은 클라이언트의 종류와 상관없이 HTTP 프로토콜을 사용하는 서블릿이 바탕이됨
> - 클라이언트와 연결돼서 동작하는 프리젠테이션 계층은 자바에서 HTTP 프로토콜을 처리하는 가장 기본 엔진이 서블릿 기술을 바탕으로함

<br>

#### 👉 계층형 아키텍처와 설계의 원칙

> - 객체지향 설계는 아키텍처 레벨의 계층과 그 관계에도 동일하게 적용 가능함
> - 각 계층은 응집도가 높으면서 다른 계층과는 낮은 결합도를 유지할 수 있어야함. 각 계층은 자신의 계층의 책임에만 충실해야함
> - 자신의 역할과 기술에만 충실한 계층을 만들면 각 계층 사이의 결합도는 자연스럽게 낮아짐
> - 각 계층이 자신의 책임에 충실하게 작성되어 있다면 필요한 그 밖의 작업은 다른 계층에 요청하게 될 것임
> - 이때는 계층 레벨에 정의한 인터페이스를 통해서 요청을 하게 되고, 계층 간에 사용되는 인터페이스 메서드에는 특정 계층의 기술이 최대한 드러나지 않게 만들어야함
> - 그렇지 않으면 계층 사이에 결합도가 높아질 뿐만 아니라, 계층 간의 기술이나 역할이 서로 침범하는 일이 일어남
> - 계층 간에 결합이 생기면 유연성이 떨어지기 때문에 각 계층의 내부 구현이 변화되면 다른 계층의 코드도 함께 수정해줘야함
> - 또한, 코드의 중복이 일어날 가능성이 높고 전체 코드를 이해하기는 힘들어짐. 오브젝트 간의 강한 결합이 있을 때 발생하는 문제와 성격이 유사하고 그 파장은 훨씬 더 심각함
> - 어떤 경우에라도 계층 사이의 낮은 결합도를 깨드리지 않도록 설계해야함
> - 당연히 계층 사이의 호출은 인터페이스를 통해 이루어져야함
> - 인터페이스를 사용하게 한다는 건 각 계층의 경계를 넘어서 들어오는 요청을 명확히 정의 하겠다는 의미임

<br>

#### 👉 애플리케이션 정보 아키텍처

> - 엔터프라이즈 시스템은 본질적으로 동시에 많은 작업이 빠르게 수행해야 하는 시스템임
> - 애플리케이션의 주요 상태정보는 클라이언트나 백엔드 시스템에 분산돼서 보관됨
> - 비교적 장기간 보관되는 상태정보는 주로 DB나 메인 프레임 같은 EIS 백엔드 사스템에 저장됨
> - 하나의 업무 작업이 여러 번의 요청과 페이지에 걸쳐 일어나는 경우에 유지돼야 하는 임시 상태정보는 클라이언트에 일시적으로 보관되기도 하고 서버의 사용자벼 세션 메모리에 저장되기도함
> - 이렇게 애플리케이션을 사이에 두고 흘러다니는 정보를 어떤 식으로 다룰지를 결정하는 일도 아키텍처를 결정할 때 매우 중요한 기준임
> - 엔터프라이즈 애플리케이션에 존재하는 정보를 단순히 데이터로 다루는 경우와 오브젝트로 다루는 경우, 두 가지 기준으로 구분할 수 있음
...
> - DB나 백엔드 시스템에서 가져온 정보를 값으로 다루고 그 값을 취급하는 코드를 만들어 로직을 구현하고 값을 그대로 프리젠테이션 계층의 뷰. 즉, 사용자가 보는 화면과 연결해주는 것 
...
> - 사용자의 이름으로 사용자 정보를 검색해서 일치하는 사용자의 아이디, 비밀번호, 이름, 가입일자를 보여주는 작업이 있다고 가정 
> - 해당 작업이 하나의 업무 단위가 되면 모든 계층의 코드가 이 기준에 맞춰서 만들어짐. 즉, 사용자 조회라는 단위 업무를 위해서만 존재하는 각 계층의 코드가 만들어짐
> - SQL의 결과를 그대로 웹 페이지의 정보 필드에 1:1 로 매핑해서 넣어줄 수 있기 때문임. 결국 SQL은 이미 화면에 어떤식으로 출력이 될지 알고 있어야 하는 상황임
> - 모든 계층의 코드는 '이름을 이요한 고객 조회'라는 업무에 종속됨. 또한, 업무의 내용이 바뀌면 모든 계층의 코드가 함께 변경됨
> - 종속적일 뿐만 아니라 배타적이어서 다른 단위 업무에 재사용하기 어려움
> - 업무 트랜잭션 단위로 코드를 묶어서 만들지 기능을 세분화해서 분리하고 재사용하지 않음
> - 하나의 업무 트랜잭션을 모두 담은 서비스 계층 코드와 해당 업무에 특화된 SQL을 담은 하나 또는 여러개 DAO 메서드로 구성됨 
> - 이런 방식은 자바 코드를 단지 DB와 웹 화면을 연결해주는 단순한 인터페이스 도구로 전락시킴
> - DB 중심의 업무 단위로 코드를 만들면 애플리케이션 내에서 흘러 다니는 정보는 항상 단순한 포맷의 데이터
> - 로직을 DB보다는 애플리케이션으로 가져오는 편이 유리함. 비용도 저렴해질 뿐 아니라 안전성도 높아지고 코드를 검증하기 매우 편함
> - DB에는 부하를 가능한 주지않는 간단한 작업만 하고 복잡한 로직은 오브젝트에 담아서 애플리케이션 내에 처리하도록 만드는 편이 좋음
...
> - <거대 서비스 계층 방식의 아키텍처>
> - DB에는 부하가 걸리지 않도록 저장 프로시저의 사용을 자제하고 복잡한 SQL을 피하면서 주요 로직은 서비스 계층의 코드에서 처리하도록 만듦
> - 거대한 서비스 계층에서는 DAO에서 좀 더 단순한 결과를 돌려줌
> - DAO가 돌려준 정보를 분석, 가공하면서 비즈니스 로직을 적용하는 것은 서비스 계층 코드의 책임이 됨
> - DAO와 SQL은 상대적으로 단순해지고 그중 일부는 여러 서비스 계층 코드에서 재사용이 가능해짐
> - 상대적으로 단순한 DAO 로직을 사용하고, 비즈니스 로직의 대부분을 서비스 계층에 집중하는 방식은 결국 '거대한 서비스 계층'을 만듦
> - 거대 서비스 계층의 방식의 장점은 아래와 같음
>   - (1) 애플리케이션 코드에 비즈니스 로직이 담겨 있기 때문에 자바 언어의 장점을 활용해서 로직을 구현할 수 있고 테스트하기도 수월함
>   - (2) 각 단위 업무별로 독립적인 개발이 가능하므로 초기 개발 속도가 빠르고, 개발자 사이에 간섭없이 독립적인 개발이 가능함. 또한, 핵 로직이 자바 코드안에 담겨 있으므로 테스트하기 수월함
...
```java
// Category 오브젝트를 사용하는 메서드 

```


> - 오브젝트 중심 아키텍처가 데이터 중심 아키텍처와 다른 가장 큰 특징은 '도메인 모델을 반영하는 오브젝트 구조를 만들어두고 그것을 각 계층 사이에서 정보를 전송하는데 사용함'
> - 그래서, 객체지향 분석과 모델링의 결과로 나오는 도메인 모델을 오브젝트 모델로 활용함
> - 오브젝트를 만들어두고 오브젝트 구조 안에 정보를 담아서 각 계층 사이에 전달하게 만드는 것이 오브젝트 중심 아키텍처임
> - 오브젝트 방식에서는 애플리케이션에서 사용되는 정보가 도메인 모델의 구조를 반영해서 만들어진 오브젝트 안에 담김
> - 도메인 모델은 애플리케이션 전 계층에서 동일한 의미를 갖음
> - 따라서, 도메인 모델이 반영된 도메인 오브젝트도 전 계층에서 일관된 구조를 유지한 채로 사용될 수 있음
> - 도메인 오브젝트는 단순히 특정 SQL에 대응되는 맵과 배열, 매번 달라지는 SQL 결과를 담기 위해 급조해서 만든 오브젝트와 달리 애플리케이션 어디에서든 사용될 수 있는 일관된 형식의 도메인 정보를 담고 있음
...
> - 도메인 오브젝트 사용에도 단점이 있음
> - 최적화된 SQL울 매번 만들어 사용하는 경우에 비해 성능 면에서 손해를 감수해야함
> - 최적화를 고려해서 DAO로 작성하려면 DAO는 비즈니스 로직에서 각 오브젝트를 어디까지 사용해야 하는지 어느정도 알고 있어야함
> - 그래서, 데이터 중심 접근 방법의 단점이라고 봤던 DAO와 비즈니스 로직 코드의 결합도가 높아지는 문제가 발생할 수도 있음
> - 위의 문제를 해결하는 방법에는 '지연로딩' 이라는 방식이 있음
>   - 최소한의 오브젝트 정보만 읽어두고 관계하고 있는 오브젝트가 필요한 경우에만 동적으로 DB에서 다시 읽어올 수 있음
>   - 이상적인 방법은 JPA, 하이버네이트 같은 ORM 프레임워크를 사용하는 것
>   - 이런 데이터 액세스 기술은 기본적으로 지연된 로딩 기법 등을 제공해주기 때문에 번거로운 코드를 만들지 않고도 도메인 오브젝트의 생성을 최적화할 수 있음
>   - 또한, 내부적으로 최적화된 SQL을 사용하도록 튜닝 가능함
>   - 또, 자주 변경되지 않으면서 많은 로직에서 참조하는 래퍼런스 테이블이 있다면 이를 ORM이 제공하는 오브젝트 캐시에 담아두고 사용할 수도 있음
>   - 오브젝트 중심 아키텍처에서는 가능하면 오브젝트 중심 데이터 액세스 기술을 사용하는 것이 좋음
...
> - <빈약한 도메인 오브젝트 방식>
> - 빈약한 오브젝트란? 도메인 오브젝트에 정보만 담고 있고 정보를 활용하는 아무런 기능도 갖고 있지 않는 오브젝트를 말함
> - 물론, 빈약한 오브젝트가 가치가 없는 건 아님. 애플리케이션 전 계층에서 해당 오브젝트를 일관되게 사용할 수 있다는 것 자체가 유용함
> - 도메인 오브젝트에는 도메인의 비즈니스 로직을 담을 수 있음
> - 위의 그림에서는 도메인 오브젝트는 3개의 계층에는 독립적으로 존재하면서 일관된 구조의 정보를 담아서 계층 간에 전달하는 데 사용됨 
...
> - <풍성한 도메인 오브젝트 방식>
> - 해당 방식은 빈약한 도메인 오브젝트의 단점을 극복하고 도메인 오브젝트의 객체지향적인 특징을 잘 사용할 수 있도록 개선한 형태
> - 어떤 비즈니스 로직은 특정 도메인 오브젝트나 그 관련 오브젝트가 가진 정보와 깊은 관계가 있음
> - 위와 같은 로직을 서비스 계층의 코드가 아니라 도메인 오브젝트에 넣어주고, 서비스 계층의 비즈니스 로직에서 재사용하게 만드는 것
> - 이렇게 도메인 오브젝트 안에 로직을 담아두면 이 로직을 서비스 계층의 메서드에 따로 만드는 경우보다 응집도가 높음
> - 데이터와 그것을 사용하는 기능이 한 곳에 모여 있기 때문임
> - 위의 첫번째 코드는 복잡함. 그 이유는 이미 정보를 담고있는 오브젝트가 있지만 그 정보를 다루는 메서드는 별개의 서비스 오브젝트에 분리되어 있기 때문임
> - 하지만, 두번째 코드는 Category 오브젝트에게 직접 필요한 계산 작업을 요청해서 처리하기 때문에 훨씬 간단함
> - 풍성한 도메인 오브젝트 방식은 실제 작성된 코드를 살펴보면 훨씬 간결하고 객체지향적임
> - 도메인 오브젝트는 직접 데이터 액세스 계층이나 기반 계층 또는 다른 서비스 계층의오브젝트에 접근할 수 없기 때문에 서비스 계층이 필요함
> - 도메인 오브젝트는 스프링 컨테이너가 관리하는 오브젝트, 즉 빈이 아님
> - 따라서, 도메인 오브젝트는 애플리케이션의 코드 또는 기타 프레임워크나 라이브러리, JDBC, 템플릿 등에 의해 필요할 때마다 새로 만들어짐
> - 결국 도메인 오브젝트는 DAO나 서비스 오브젝트 같은 빈의 기능을 사용할 수 없음
> - 스프링의 빈으로 관리되는 3계층의 오브젝트들은 도메인 오브젝트를 자유롭게 이용할 수 있지만 그 반대는 안됨

<br>

#### 👉 도메인 계층 방식

> - 도메인 계층의 역할과 비중을 극대화하려다 보면 기존의 풍성한 도메인 오브젝트 방식으로는 어려움
> - 도메인 오브젝트가 기존 3계층과 같은 레벨로 격상되어 하나의 계층을 이루는 방식을 '도메인 계층 방식'이라고함
> - 도메인 계층 방식이란? 도메인 오브젝트들이 하나의 독립적인 계층을 이뤄서 서비스 계층과 데이터 액세스 계층 사이에 존재하는 것
> - 도메인 계층 방식에는 2가지 특징이 있음 
>   - (1) 도메인에 종속적인 비즈니스 로직의 처리는 서비스 계층이 아니라 도메인 계층의 오브젝트 안에서 진행됨
>   - (2) 도메인 오브젝트가 기존 데이터 액세스 계층이나 기반 계층의 기능을 직접 활용할 수 있음 
>   
> - 도메인 오브젝트는 스프링이 직접 만들고 관리하는 오브젝트가 아님. 하지만, 스프링이 관리하지 않는 오브젝트에도 DI 적용할 수 있음. 이 과정에서 AOP가 적용됨
> - 스프링 AOP 대신 AspectJ AOP를 사용하면 클래스의 생성자가 호출되면서 오브젝트가 만들어지는 시점을 조인 포인트로 사용할 수 있고 스프링 빈이 아닌 일반 오브젝트에도 AOP 부가기능을 적용할 수 있음 
> - 스프링이 직접 관리하지 않는 오브젝트에 대한 DI 서비스가 일종의 AOP 부가기능으로 도메인 오브젝트에 적용될 수 있음
> - 해당 방식을 이용하면 도메인 오브젝트가 생성될 때 스프링의 빈 오브젝트를 DI 받게할 수 있음
> - 도메인 계층은 사용해도 서비스 계층의 역할은 존재함
> - 여러 도메인 오브젝트의 기능을 조합해서 복잡한 작업을 적용해야 하는 경우가 있음
> - 특정 도메인 오브젝트에 담길 수 없는 이런 작업은 서비스 계층에서 도메인 계층과 협력을 통해 진행하는 것이 바람직함
> - 또는 굳이 도메인 계층을 거치지 않고 바로 데이터 액세스 계층으로 부터 정보를 가져와 클라이언트에 제공해야 하는 경우도 있음
> - 도메인 오브젝트를 독립적인 계층으로 만들려고 할 때 고려해야 할 사항이 있음
> - 도메인 오브젝트가 도메인 계층을 벗어나서도 사용되게 할지 말지를 결정해야함
>   - (1) 모든 계층에서 도메인 오브젝트를 사용함
>     - 해당 방식은 위험함. 도메인 오브젝트의 메서드는 중요한 도메인/비즈니스 로직을 담당함
>     - 따라서, 도메인 오브젝트를 프리젠테이션 계층 ... 에서 함부로 사용하는 위험이 뒤 따를 수 있음
>     - 이러한 문제는 코딩 정책의 적용을 분석할 수 있는 툴을 이용해 검증하거나 AspectJ 의 정책/표준 강제화 기능을 사용하면됨
>   
>   - (2) 도메인 오브젝트는 도메인 계층을 벗어나지 못하게함
>     - 도메인 계층 밖으로 전달될 때는 별도로 준비된 전달용 오브젝트에 도메인 오브젝트의 내용을 복사해서 넘겨줘야함
>     - DTO는 상태의 변화를 허용하지 않고 읽기 전용으로 만들어지기도함
>   
> - 도메인 계층은 기존 3 계층과 다른 특징이 있음
> - 서비스를 제공하는 싱글톤으로 계속 존재하는 다른 계층의 오브젝트와 달리, 도메인 계층의 오브젝트는 매우 짧은 시간동안만 존재했다가 사라지는 것을 반복함
> - 도메인 오브젝트는 사용자별 요청에 대해 독립적인 상태를 유지하고 있어야함

<br>

#### 👉 DTO 와 리포트쿼리

> - 도메인 계층 방식의 경우 도메인 계층을 벗어난 정보를 DTO라 불리는 특정 게층에 종속되지 않는 정보 전달의 목적을 가진 단순한 오브젝트에 담아 사용하기도함
> - 대표적으로 리포트쿼리라고 불리는 DB 쿼리의 실행 결과를 담는 경우가 있음

<br>

#### 👉 계층형 아키텍처

> - 스프링을 처음 학습하고 도입하는 입장이라면 가장 전통적인 방식인 서버 기반의 3계층 구조에 먼저 익숙해지는 것이 좋음
> - 프리젠테이션 계층은 SpringMVC를 이용하고, 서비스 계층은 POJO로 구현하면서 트랜잭션 AOP를 적용하고, 데이터 액세스 계층은 JDBC, JPA, ... 을 활용하도록하는 것
> - 처음에는 이 3계층 방식의 아키텍처 위에서 스프링의 철학과 가치를 따라서 코드를 만드는 데 익숙해진 다음 다양한 방식으로 계층 구조의 통합과 분산을 시도하는 게 좋음
...
> - 아키텍처 설계에서 한 가지 더 신경써야 할 사항은 '상태관리'이다.
> - 하나의 애플리케이션이 동시에 수 많은 사용자의 요청을 처리하게 하기 위해 매번 간단한 요청을 받아서 그 결과를 돌려주는 방식으로 동작함
> - 따라서, 서버의 자원이 특정 사용자에게 일정하게 할당되지 않음
> - 그래서 서버 기반의 애플리케이션은 원래 지속적으로 유지되는 상태를 갖지 않음(stateless)
> - 어떤 식으로든 애플리케이션의 상태와 장시간 진행되는 작업 정보는 유지돼야함
> - 이를 위해 웹 클라이언트에 URL, 파라미터, 폼 히든 필드, 쿠키 등을 이용해 상태정보 또는 서버에 저장된 상태정보에 키 값 등을 전달해야함
> - 클라이언트와 서버 사이에서 많은 양의 정보를 계속해서 주고 받을 수 없으므로 중요한 상태 정보는 파일 시스템, 데이터그리드, DB 등에 저장되기도함
> - 또는 제약이 있기는 하지만 HTTP 세션과 같은 서블릿 컨테이너가 제공하는 저장공간을 활용하기도함
> - 스프링은 기본적으로 상태가 유지되지 않는 빈과 오브젝트를 사용하는 것을 권장함
> - 스프링을 이용해서 상태유지 stateful 스타일의 애플리케이션을 얼마든지 만들 수 있음
> - 빈의 스코프를 잘 활용하면, 스프링이 관리하는 빈이면서 사용자별로 또는 단위 작업별로 독립적으로 생성되고 유지되는 오브젝트를 만들어 상태를 저장하고 이를 DI를 통해 서비스 빈에서 사용하게 만들 수 있음

<br>

#### 👉 서드파티 프레임워크, 라이브러리 적용

> - 표준기술 외에도 오픈소스 프레임워크, 라이브러리, ... 스프링과 함께 사용 가능함
> - 스프링이 지원하는 기술에 대해 알아봄
>   - (1) 해당 기술을 스프링의 DI 패턴을 따라 사용할 수 있음
>     - 프레임워크의 핵심 오브젝트를 빈의 형태로 등록해둘 수 있다면 프로퍼티를 이용해 세부 설정을 조정할 수도 있고, DI를 통해 다른 오브젝트에서 손 쉽게 활용할 수도 있음
>     - 또 스프링이 제공하는 추상화 서비스를 통해 다른 리소스에 투명하게 접근할 수도 잇음
>     - 스프링 외의 기술을 접목할 때는 가장 먼저 스프링의 빈으로 등록해서 DI 방식을 통해 사용가능한지 살펴봐야함
>     - 만약 빈으로 등록해서 바로 사용할 수 있는 구조로 핵심 API나 클래스가 만들어져 있지 않는 경우라면, 스프링 빈으로 등록돼서 사용하기에 적합하도록 만들어주는 팩토리 빈을 도입해야함
> 
>   - (2) 스프링의 서비스 추상화가 적용됐다
>     - 첫번째 방법은 사용할 기술을 스프링의 빈으로 등록하고 설정 가능하도록 지원해주는 것
>     - 사용 기술의 API는 애플리케이션에 그대로 노출됨
>     - 서비스 추상화를 적용하는 경우는 이보다 한 발 더 나아가서 비슷한 기능을 제공하는 기술에 대한 일관된 접근 방법을 정의해줌
>     - 이를 통해 서드파티 프레임워크를 적용할 수있을 뿐만 아니라 필요에 따라 호환 가능한 기술로 손쉽게 교체해서 사용할 수 있음
>     - 다양한 비표준 기술과 영역에 확장해서 적용한 것이 바로 스프링의 서비스 추상화
>     - 개발자에게는 표준 기술을 사용하듯이 일관된 방법으로 코드를 작성하게 해준다는 것은 중요한 의미가 있음
>   
>   - (3) 스프링이 지지하는 프로그래밍 모델을 적용함
>     - 스프링의 데이터 액세스 지원 기능을 사용하면 데이터 액세스 기술의 종류에 상관없이 일관된 예외 계층 구조를 따라서 예외가 던져짐
>     - 이는 스프링의 개발 철학이 적용된 것임
>     - 이를 통해, 서비스 계층의 비즈니스 로직을 담은 코드가 데이터 액세스 계층의 기술에 종속되지 않도록 만듦
>   
>   - (4) 템플릿/콜백이 지원됨
>     - 여느 기술 중에는 그대로 사용하면 반복적으로 등장하는 판에 밖힌 코드 때문에 전체 코드가 지저분해지고 이해하기 힘들고 추상화하기도 어려운 구조가 돼버림
>     - 스프링은 이런 기술을 간편하게 사용할 수 있도록 '템플릿/콜백' 기능을 제공함
>     - 대부분의 템플릿 클래스는 빈으로 등록해서 필요한 빈에서 DI 받아 사용할 수 있음
>   
> - 스프링에 새로운 기술을 연동하려면 앞서 설명한 스프링의 프로그래밍 모델과 지지하는 개발철학을 따르면서 앞에서 설명한 네가지 방법을 이용하면됨
> - 스프링을 사용하려면 스프링의 프로그래밍 모델과 그에 담긴 개발 철학을 따르는 일관된 코드르 만드는 데 많은 관심을 기울어야함
